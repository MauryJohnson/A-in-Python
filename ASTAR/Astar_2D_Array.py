...
###############################################################################################################################################################################################
    ...
    if(Goal[0]<0 or Goal[0]>len(ENV)-1 and( Goal[1]<0 or Goal[1]>len(ENV[0])-1)):
	print("Goal is Out of bounds")
	return
    
    #ENV[Goal[0]][Goal[1]] = 1
    if(ENV[Goal[0]][Goal[1]]==0):
	print("Goal inside of obstacle")
	return
    ...
    if((S[0]<0 or S[0]>len(ENV)-1) and( S[1]<0 or S[1]>len(ENV[0])-1)):
    ...
    if(ENV[S[0]][S[1]]==0):
	print("Start inside of obstacle")
	return
