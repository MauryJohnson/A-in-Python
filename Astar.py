...
#Remove Redundancies to have fully optimal solution
...
def UpdateVertex(s1,s2):
    ...
    #Check if found a shorter path
    print ("VALUE:",First[2]+C, "<", s2[2], "??")
	...
	#If the fringe already contains s2, remove it	
	if(F.Exists(s2)):
	    F.Remove(s2[0],s2[1])

	s2[2] = s2[2] + Heuristic(s2)
...
def NonCollisions(S):
	...
	GoTo.append([TempY,TempX,Cost(S,[TempY,TempX])+Heuristic([TempY,TempX]),StartY,StartX])
...
def main():
    ...
    Goal = [int(sys.argv[3]),int(sys.argv[4]),0,int(sys.argv[3]),int(sys.argv[4])]
    ...
    S = [int(sys.argv[1]),int(sys.argv[2]),0,int(sys.argv[1]),int(sys.argv[2])]
    ...
    Ss = [int(sys.argv[1]),int(sys.argv[2]),0,0,0]
	...
	for u in L:
		...
		print("U:",u," is not visited YET")
		if not F.Exists(u):
		    print("U:",u," is not in fringe YET")
		    O = u
		    ENV[u[0]][u[1]] = 3
		    PrintE()
