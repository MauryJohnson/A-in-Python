...
def Heuristic(s1):
    ...
    #Trace A* Heuristic
    return ((math.sqrt(2)*min(abs(End[0]-Start[0]),abs(End[1]-Start[1])) + max(abs(End[0]-Start[0]),abs(End[1]-Start[1])) - min(abs(End[0]-Start[0]),abs(End[1]-Start[1]))))
...
def Execute(EE,SY,SX,GY,GX):
    ...
    #if(len(sys.argv[:])<5 and ST==sys.argv[:]):
    ...
    #Y = int(ST[1])
    #sys.argv = ST[:]
	##print ("ALL ARGS FROM OUTSOURCE:",sys.argv)
	#return
    #else:
	#f = open('maze','r')
        #x = f.read().splitlines()
